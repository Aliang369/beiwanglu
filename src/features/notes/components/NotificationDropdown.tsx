...
export function NotificationDropdown({ messages, onMessageOpen, onViewAll, onMarkAllRead }: NotificationDropdownProps) {
  ...
  function handleMarkAllReadClick(event: MouseEvent<HTMLButtonElement>) {
    event.stopPropagation()
    onMarkAllRead?.()
  }
  ...
  return (
    <div className="absolute top-full right-0 z-40 mt-2 w-80 overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest shadow-lg">
      <div className="flex items-center justify-between border-b border-outline-variant bg-surface-container-low px-4 py-3">
        ...
        <button type="button" onClick={handleMarkAllReadClick} className="font-label-sm text-label-sm text-primary transition-colors hover:underline">
          全部标记为已读
        </button>
      </div>
      ...
      <div className="max-h-[300px] overflow-y-auto">
        {previewMessages.length === 0 ? (
          <p className="px-4 py-6 text-center font-label-sm text-label-sm text-on-surface-variant">暂无通知</p>
        ) : (
