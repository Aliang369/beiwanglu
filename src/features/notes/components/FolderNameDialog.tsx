...
export function FolderNameDialog({
  ...
  const resolvedTitle = title ?? (mode === 'create' ? '新建文件夹' : '重命名文件夹')
  const resolvedDescription =
    description ?? (mode === 'create' ? '给分类起一个清晰的名字，方便之后整理笔记。' : '修改名称后，列表会立刻更新显示。')
  const resolvedSubmitLabel = submitLabel ?? (mode === 'create' ? '创建' : '保存')
  ...
  const unchanged = mode === 'rename' && trimmed === initialTrimmed
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const conflict =
      mode === 'rename'
        ? existingNames.some((existing) => existing === trimmed && existing !== initialTrimmed)
        : existingNames.some((existing) => existing === trimmed)
      ...
      >
        <form onSubmit={handleSubmit}>
          <div className="px-6 pb-2 pt-6">
            ...
            <p className="mt-1 font-body-md text-body-md text-on-surface-variant">{resolvedDescription}</p>
          </div>

          <div className="px-6 py-4">
            <input
              ref={inputRef}
              id={mode === 'create' ? 'folder-name' : 'rename-folder-name'}
              type="text"
              value={name}
              onChange={(event) => {
          ...
          <div className="flex items-center justify-end gap-3 px-6 pb-6 pt-2">
            ...
            <button
              type="submit"
              disabled={mode === 'rename' && (!trimmed || unchanged)}
...
export function CreateFolderDialog({
  ...
  return <FolderNameDialog mode="create" existingNames={existingNames} onClose={onClose} onSubmit={onCreate} />
}

/** 兼容旧 import：RenameFolderDialog */
export function RenameFolderDialog({
  ...
  return (
    <FolderNameDialog mode="rename" initialName={initialName} existingNames={existingNames} onClose={onClose} onSubmit={onRename} />
  )
}
